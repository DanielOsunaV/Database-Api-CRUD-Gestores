...
//Controller contains the interaction methods with the Database, here you have the basics for a CRUD application
        ...
        public ActionResult Put(int id, [FromBody] Gestores_Bd gestor)
        {
            try
            {
                if (gestor.id == id)
                {
                    ...
                    context.SaveChanges();
                    return CreatedAtRoute("GetGestor", new { id }, gestor);
                }
                else
                {
                    return BadRequest();
